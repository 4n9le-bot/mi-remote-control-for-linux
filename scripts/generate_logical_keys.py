...
def main():
    ...
    license_line = next(iter(text.splitlines()), "").strip()
    expected_license = "/* SPDX-License-Identifier: GPL-2.0-only WITH Linux-syscall-note */"
    if license_line != expected_license:
        ap.error("header does not carry the expected Linux UAPI license")
    digest = hashlib.sha256(text.encode()).hexdigest()
    lines = ["// Generated by scripts/generate_logical_keys.py; do not edit.", "use super::LogicalKey;", "", f'pub const REGISTRY_LINUX_TAG: &str = "{args.linux_tag}";', f'pub const REGISTRY_SOURCE_URL: &str = "{args.source_url}";', f'pub const REGISTRY_SOURCE_SHA256: &str = "{digest}";', 'pub const REGISTRY_LICENSE: &str = "GPL-2.0-only WITH Linux-syscall-note";', f'pub const REGISTRY_CATALOG_VERSION: u32 = {args.catalog_version};', "", "pub static LOGICAL_KEYS: &[LogicalKey] = &[" ]
